...
export const uniUtils = {
  ...
  async messageAudioTranslate(body: {
    fileName: string;
    type: string;
  ...
  getLangCodeByChannel(lang: string, channel: string) {
    const uniStoreState = uniStore.getState();
    const langConfig = uniStoreState.langMap[lang];
    const langItem = langConfig?.channelCodeList?.find(
      (i: any) => i.channel === channel
    );
    return langItem?.abrige ?? lang;
  },
};
export const uniHttpApi = {
  ...
  translateAudio: ({ body }: any) =>
    uniUtils.fetch({
      method: 'POST',
      url: `client/translate/translationSpeechZzz`,
      body,
    }),
  getLoginUser: () =>
    uniUtils.fetch({
